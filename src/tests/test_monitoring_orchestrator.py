...
from cmi.core.plans import DEFAULT_TOKEN_LIMIT
...
def orchestrator(
    ...
    with (
        patch(
            "cmi.monitoring.orchestrator.DataManager",
            return_value=mock_data_manager,
        ),
        patch(
            "cmi.monitoring.orchestrator.SessionMonitor",
...
class TestMonitoringOrchestratorInit:
    ...
    def test_init_with_defaults(self) -> None:
        ...
        with (
            patch("cmi.monitoring.orchestrator.DataManager") as mock_dm,
            patch("cmi.monitoring.orchestrator.SessionMonitor") as mock_sm,
        ):
    ...
    def test_init_with_custom_params(self) -> None:
        ...
        with (
            patch("cmi.monitoring.orchestrator.DataManager") as mock_dm,
            patch("cmi.monitoring.orchestrator.SessionMonitor"),
        ):
            orchestrator = MonitoringOrchestrator(
...
class TestMonitoringOrchestratorLifecycle:
    ...
    def test_start_monitoring_already_running(
        ...
        with patch("cmi.monitoring.orchestrator.logger") as mock_logger:
            orchestrator.start()
...
class TestMonitoringOrchestratorFetchAndProcess:
    ...
    def test_fetch_and_process_success(
        ...
        with patch(
            "cmi.monitoring.orchestrator.get_token_limit",
            return_value=200000,
        ):
    ...
    def test_fetch_and_process_callback_error(
        ...
        with (
            ...
            patch("cmi.monitoring.orchestrator.report_error") as mock_report,
        ):
            result = orchestrator._fetch_and_process_data()
